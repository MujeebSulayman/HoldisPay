...
import { FormError, FormActions } from '@/components/form';
...
export default function AdminCreateInvoicePage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (expiry < tomorrow) {
        setError('Expiry date must be tomorrow or later.');
        setIsSubmitting(false);
        return;
      }
      ...
      <div className="max-w-2xl mx-auto py-8">
        <div className="bg-gray-900/50 border border-gray-800 rounded-lg p-6 sm:p-8">
          ...
          <div className="flex flex-col gap-3 sm:flex-row">
            <button
              type="button"
              onClick={() => router.push(`/admin/invoices/${createdInvoiceId}`)}
              className="w-full sm:flex-1 py-3 bg-teal-500 hover:bg-teal-600 text-white font-medium rounded-lg"
            >
              View invoice
            </button>
            <button
              ...
              className="w-full sm:flex-1 py-3 bg-gray-800 hover:bg-gray-700 text-white font-medium rounded-lg"
            >
              View all invoices
            </button>
          </div>
  ...
  return (
    <div className="w-full max-w-4xl mx-auto py-4 sm:py-6 md:py-8 min-w-0">
      <div className="mb-4 sm:mb-6">
        ...
        <p className="text-gray-400 text-sm">Create an invoice as admin. Customer pays via secure link; funds are held in escrow until completion.</p>
      </div>
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
                  ...
                  <div key={item.id} className="grid grid-cols-12 gap-2 items-center">
                    <div className="col-span-5 sm:col-span-6">
                      <input
                        ...
                        className={inputClass}
                        placeholder="e.g. Consulting, design work"
                      />
                    </div>
        ...
        <div className="bg-gray-900/50 border border-gray-800 rounded-lg p-5">
          ...
          <div className="grid sm:grid-cols-3 gap-4">
            ...
            <div>
              <label className={labelClass}>Expiry date <span className="text-amber-400">*</span></label>
              <DatePicker
                value={dueDate}
                onChange={setDueDate}
                minDate={(() => {
