...
export interface InvoiceDetailTemplateProps {
  invoice: Invoice;
  backHref: string;
  backLabel: string;
...
export function InvoiceDetailTemplate({
  ...
  if (invoice.line_items && Array.isArray(invoice.line_items) && invoice.line_items.length > 0) {
    lineItemsForTable = invoice.line_items.map((item: { description?: string; quantity?: string | number; unitPrice?: string | number; amount?: number }) => {
  ...
  const fromName = invoice.business_name || issuerDisplayName || 'Your business';
        ...
        <div className="bg-white rounded-lg shadow-sm border border-gray-200 py-[10px] overflow-x-hidden">
          ...
          <div className="grid grid-cols-1 gap-6 px-4 py-5 border-t border-gray-100 sm:grid-cols-2 sm:gap-8 sm:px-5 sm:py-6 md:px-8">
            <div className="min-w-0">
              ...
              <p className="font-medium text-gray-900 break-words">{fromName}</p>
              {invoice.business_address && <p className="text-sm text-gray-600 mt-0.5 break-words">{invoice.business_address}</p>}
              {issuerEmail && <p className="text-sm text-gray-600 mt-0.5 break-all">{issuerEmail}</p>}
            </div>
            <div className="min-w-0">
