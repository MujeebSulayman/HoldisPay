...
export interface WalletOverviewFlow {
	...
	flowByType?: {
		inflow: {
			deposit: string;
			receiver_payment: string;
			payment_link_deposit: string;
		};
		outflow: {
			withdraw: string;
			invoice_fund: string;
			user_withdrawal: string;
		};
	};
	byChain: Array<{
		chainId: string;
...
export const userApi = {
	...
	async getWalletOverview(
		userId: string,
		params?: { periodsWeeks?: number; recentLimit?: number },
	) {
		const search = new URLSearchParams();
		if (params?.periodsWeeks != null)
			search.set('periodsWeeks', String(params.periodsWeeks));
		if (params?.recentLimit != null)
			search.set('recentLimit', String(params.recentLimit));
