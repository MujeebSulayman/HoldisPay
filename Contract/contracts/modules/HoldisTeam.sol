...
contract HoldisTeam is Initializable, AccessControlUpgradeable, PausableUpgradeable {
    ...
    event TeamMemberAdded(uint256 indexed contractId, address indexed member, uint256 sharePercentage, string role);
    event TeamMemberRemoved(uint256 indexed contractId, address indexed member);
    event BonusAdded(uint256 indexed contractId, uint256 indexed bonusId, uint256 amount, string description);
    event BonusPaid(uint256 indexed contractId, uint256 indexed bonusId, address recipient, uint256 amount);
    ...
    function addTeamMember(uint256 contractId, address member, uint256 sharePercentage, string memory role) external whenNotPaused {
    ...
    function removeTeamMember(uint256 contractId, address member) external whenNotPaused {
        require(paymentsContract.isEmployer(contractId, msg.sender), "Not employer");
    ...
    function addBonus(uint256 contractId, string memory description, uint256 amount, string memory kpiHash) external whenNotPaused returns (uint256) {
        require(paymentsContract.isEmployer(contractId, msg.sender), "Not employer");
    ...
    function markBonusPaid(uint256 contractId, uint256 bonusId, address recipient) external onlyRole(ADMIN_ROLE) {
        PerformanceBonus storage bonus = bonuses[contractId][bonusId];
